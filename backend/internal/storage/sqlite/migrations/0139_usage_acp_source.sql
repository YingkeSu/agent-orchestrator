...
CREATE TABLE model_usage_events_next (
    ...
    -- 0116 and 0138 appended these columns physically last; the rebuild
...
-- Drop what the narrow CHECKs cannot represent before restoring them, the
-- same way 0117 dropped Kimi rows on downgrade.
DELETE FROM model_usage_events
WHERE usage_source_id IN (
    SELECT us.id FROM usage_sources us WHERE us.kind = 'acp_usage'
);
DELETE FROM usage_sources WHERE kind = 'acp_usage';
DELETE FROM usage_bindings WHERE harness NOT IN ('claude-code', 'codex', 'kimi');
