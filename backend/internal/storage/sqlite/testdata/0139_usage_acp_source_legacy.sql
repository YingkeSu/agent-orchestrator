...
-- ACP-delivered usage (worker harnesses such as opencode) becomes a certified
-- source. The facts are archived in conversation_provider_events, not in a
-- provider-owned transcript, so the binding needs the session's harness
-- vocabulary and the source needs its own kind. Same structural rebuild as
-- 0117: CHECK constraints cannot be altered in place.
